...
- (void)hx_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(nullable void *)context{
    ...
    */
    //- 保存多个信息
    HXKVOInfo *info = [[HXKVOInfo alloc] initWithObserver:observer forKeyPath:keyPath options:options];
    //使用数组存储 -- 也可以使用map
    ...
    BOOL isAutomatically = [self hx_performSelectorWithMethodName:@"automaticallyNotifiesObserversForKey:" keyPath:keyPath];
    ...
    //添加一个setter方法
    class_addMethod(newClass, setterSel, (IMP)hx_setter, type);
...
- (void)hx_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath handleBlock:(LGKVOBlock)block{
    ...
    */
    //- 保存多个信息
    HXKVOInfo *info = [[HXKVOInfo alloc] initWithObserver:observer forKeyPath:keyPath handleBlock:block];
    //使用数组存储 -- 也可以使用map
    NSMutableArray *mArray = objc_getAssociatedObject(self, (__bridge const void * _Nonnull)(kCJLKVOAssociateKey));
    if (!mArray) {//如果mArray不存在，则重新创建
        mArray = [NSMutableArray arrayWithCapacity:1];
...
- (Class)createChildClassWithKeyPath:(NSString *)keyPath
{
    ...
    class_addMethod(newClass, classSel, (IMP)hx_class, classType);
    
    //添加dealloc 方法
    SEL deallocSel = NSSelectorFromString(@"dealloc");
    Method deallocMethod = class_getInstanceMethod([self class], deallocSel);
    const char *deallocType = method_getTypeEncoding(deallocMethod);
    class_addMethod(newClass, deallocSel, (IMP)hx_dealloc, deallocType);
    
    return newClass;
}
#pragma mark - 重写setter方法，向父类发消息（响应式编程 / 函数式编程）
static void hx_setter(id self, SEL _cmd, id newValue){
    NSLog(@"来了:%@",newValue);
...
Class hx_class(id self, SEL _cmd){
...
void hx_dealloc(id self, SEL _cmd){
    NSLog(@"来了 %s", __func__);
    Class superClass = [self class];
    object_setClass(self, superClass);
}
...
/// @param keyPath 观察属性
- (BOOL)hx_performSelectorWithMethodName:(NSString *)methodName keyPath:(id)keyPath {
...
#pragma mark - 方法交换封装
+ (BOOL)hx_hookOrigInstanceMenthod:(SEL)oriSEL newInstanceMenthod:(SEL)swizzledSEL {
    Class cls = self;
