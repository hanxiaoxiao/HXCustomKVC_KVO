...
#import "NSObject+HXKVC.h"
#import <objc/runtime.h>

@implementation NSObject (HXKVC)
//设值
-(void)hx_setValue:(nullable id)value forKey:(NSString*)key{
    ...
    //    2、找setter方法，顺序是：setXXX、_setXXX、 setIsXXX
        ...
        if ([self hx_performSelectorWithMethodName:setKey value:value]) {
            NSLog(@"*************%@*************", setKey);
            return;
        }else if([self hx_performSelectorWithMethodName:_setKey value:value]){
            NSLog(@"*************%@*************", _setKey);
            return;
        }else if([self hx_performSelectorWithMethodName:setIsKey value:value]){
            NSLog(@"*************%@*************", setIsKey);
            return;
        }
...
#pragma mark --相关方法--
- (BOOL)hx_performSelectorWithMethodName:(NSString *)methodName value:(id)value{
